...
fight boss player = if hitsBoss <= hitsPlayer then player else boss
...
solve weaponStr armorStr ringStr bossLineList = minimum . map equippedCost . filter ((== "Me") . name) $ results
