...
instance Ord Reindeer where
    r1 `compare` r2 = distance r1 `compare` distance r2
...
moveOneSecond reindeers = map updateReindeer reindeers
    where updateReindeer r
...
solve seconds lineList = distance . maximum . (!! seconds) . iterate moveOneSecond $ reindeers
