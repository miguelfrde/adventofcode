...
mixCalories :: [Ingredient] -> [Int] -> Int
mixCalories ingredients mixValues = sum . map (\(ing, x) -> x * calories ing) $ zip ingredients mixValues
...
solve :: [String] -> Int
solve lineList = maximum . map (applyMix ingredients) $ okMixes
    where ingredients = map parseLine lineList
          allMixes = possibleMixes maxTeaspoons (length ingredients)
          okMixes = filter (\mix -> (mixCalories ingredients mix) == 500) allMixes
